...
class Fan:
    def __init__(self, config, default_shutdown_speed=0.):
        ...
        self.off_below = config.getfloat('off_below', default=0.,
                                         minval=0., maxval=1.)
        self.zero_speed_pwm_high = config.getboolean(
            'zero_speed_pwm_high', False)
        ...
        start_power = 1.0 if self.zero_speed_pwm_high else 0.0
        if self.zero_speed_pwm_high and shutdown_power == 0.0:
            shutdown_power = 1.0
        self.mcu_fan.setup_start_value(start_power, shutdown_power)
    ...
    def _get_pwm_value(self, value):
        # Preserve existing fan-speed semantics, but allow a high PWM signal
        # when the logical fan speed is zero.
        if self.zero_speed_pwm_high and value == 0.0:
            return 1.0
        return value
    ...
    def _apply_speed(self, print_time, value):
        ...
        self.mcu_fan.set_pwm(print_time, self._get_pwm_value(value))
